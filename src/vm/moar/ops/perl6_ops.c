...
#define MVM_gc_root_add_permanent_desc(tc, obj_ref, description)               \
  MVM_gc_root_add_permanent(tc, obj_ref)
#endif
...
static MVMCallsite one_str_callsite = {one_str_flags, 1,   1, 1, 0, 0,
                                       NULL,          NULL};

/* Dispatcher vivify_for callsite. */
static MVMCallsiteEntry disp_flags[] = {
    MVM_CALLSITE_ARG_OBJ, MVM_CALLSITE_ARG_OBJ, MVM_CALLSITE_ARG_OBJ,
...
static MVMObject *get_thrower(MVMThreadContext *tc, MVMString *type) {
  ...
  return MVM_is_null(tc, ex_hash) ? ex_hash
                                  : MVM_repr_at_key_o(tc, ex_hash, type);
}
...
static void p6init(MVMThreadContext *tc, MVMuint8 *cur_op) {
  if (!initialized) {
    ...
    /* Strings. */
    str_dispatcher =
        MVM_string_ascii_decode_nt(tc, tc->instance->VMString, "$*DISPATCHER");
    MVM_gc_root_add_permanent_desc(tc, (MVMCollectable **)&str_dispatcher,
                                   "$*DISPATCHER");
    str_vivify_for =
        MVM_string_ascii_decode_nt(tc, tc->instance->VMString, "vivify_for");
    MVM_gc_root_add_permanent_desc(tc, (MVMCollectable **)&str_vivify_for,
                                   "vivify_for");
    ...
    str_xnodisp = MVM_string_ascii_decode_nt(tc, tc->instance->VMString,
                                             "X::NoDispatcher");
    MVM_gc_root_add_permanent_desc(tc, (MVMCollectable **)&str_xnodisp,
                                   "X::NoDispatcher");
  }
}

/* Boxing to Perl 6 types. */
static void discover_create(MVMThreadContext *tc, MVMSpeshGraph *g,
                            MVMSpeshIns *ins, MVMObject *type) {
...
static void p6capturelex(MVMThreadContext *tc, MVMuint8 *cur_op) {
  ...
  if (REPR(vm_code_obj)->ID == MVM_REPR_ID_MVMCode) {
    if (((MVMCode *)vm_code_obj)->body.sf->body.outer ==
...
static MVMuint8 s_p6getouterctx[] = {MVM_operand_obj | MVM_operand_write_reg,
                                     MVM_operand_obj | MVM_operand_read_reg};
...
static MVMuint8 s_p6captureouters[] = {MVM_operand_obj | MVM_operand_read_reg,
                                       MVM_operand_obj | MVM_operand_read_reg};
...
static MVMuint8 s_p6stateinit[] = {MVM_operand_int64 | MVM_operand_write_reg};
...
static MVMuint8 s_p6setfirstflag[] = {MVM_operand_obj | MVM_operand_write_reg,
                                      MVM_operand_obj | MVM_operand_read_reg};
...
static MVMuint8 s_p6takefirstflag[] = {MVM_operand_int64 |
...
static MVMuint8 s_p6setpre[] = {MVM_operand_obj | MVM_operand_write_reg};
static void p6setpre(MVMThreadContext *tc, MVMuint8 *cur_op) {
  tc->cur_frame->flags |= RAKUDO_FRAME_PRE_FLAG;
  GET_REG(tc, 0).o = NULL;
}

static MVMuint8 s_p6clearpre[] = {MVM_operand_obj | MVM_operand_write_reg};
...
static MVMuint8 s_p6inpre[] = {MVM_operand_int64 | MVM_operand_write_reg};
...
static MVMuint8 s_p6finddispatcher[] = {MVM_operand_obj | MVM_operand_write_reg,
...
static void p6finddispatcher(MVMThreadContext *tc, MVMuint8 *cur_op) {
  ...
  while (ctx) {
    ...
    MVMROOT(tc, ctx, {
      disp_lex =
          MVM_frame_try_get_lexical(tc, ctx, str_dispatcher, MVM_reg_obj);
    });
    if (disp_lex) {
      ...
      if (!MVM_is_null(tc, maybe_dispatcher)) {
        ...
        if (!IS_CONCRETE(dispatcher)) {
          ...
          MVMROOT(tc, dispatcher, {
            MVMROOT(tc, ctx, {
              ctx_ref =
                  MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTContext);
              ...
              MVMROOT(tc, ctx_ref, {
                ...
                MVMROOT(tc, capture, {
                  p6sub =
                      MVM_frame_get_code_object(tc, (MVMCode *)ctx->code_ref);
                  MVMROOT(tc, p6sub, {
                    meth = MVM_6model_find_method_cache_only(tc, dispatcher,
                                                             str_vivify_for);
                  });
                });
              });
          ...
          *(tc->interp_cur_op) += 4; /* Get right return address. */
          MVM_args_setup_thunk(tc, res_reg, MVM_RETURN_OBJ, &disp_callsite);
          {
            ...
            srd[0] = disp_lex;
            srd[1] = res_reg;
            MVM_frame_special_return(tc, tc->cur_frame, store_dispatcher, NULL,
                                     srd, NULL);
          }
...

  {
    ...
    if (!MVM_is_null(tc, thrower)) {
      ...
      *(tc->interp_cur_op) += 4; /* Get right return address. */
      ...
      STABLE(thrower)->invoke(tc, thrower, &one_str_callsite,
                              tc->cur_frame->args);
    } else {
      MVM_exception_throw_adhoc(
          tc, "%s is not in the dynamic scope of a dispatcher",
...
static void p6decodelocaltime(MVMThreadContext *tc, MVMuint8 *cur_op) {
  MVMObject *result =
      MVM_repr_alloc_init(tc, tc->instance->boot_types.BOOTIntArray);
  ...
  MVMROOT(tc, result, {
    REPR(result)->pos_funcs.set_elems(tc, STABLE(result), result,
                                      OBJECT_BODY(result), 9);
...
static MVMuint8 s_p6staticouter[] = {MVM_operand_obj | MVM_operand_write_reg,
                                     MVM_operand_obj | MVM_operand_read_reg};
static void p6staticouter(MVMThreadContext *tc, MVMuint8 *cur_op) {
  MVMObject *code = GET_REG(tc, 2).o;
  if (!MVM_is_null(tc, code) && IS_CONCRETE(code) &&
      REPR(code)->ID == MVM_REPR_ID_MVMCode) {
    MVMStaticFrame *sf = ((MVMCode *)code)->body.sf;
    GET_REG(tc, 0).o =
        sf->body.outer ? (MVMObject *)sf->body.outer->body.static_code : NULL;
  } else {
    MVM_exception_throw_adhoc(tc, "p6staticouter requires a CodeRef");
  }
}

static MVMuint8 s_p6invokeunder[] = {MVM_operand_obj | MVM_operand_write_reg,
                                     MVM_operand_obj | MVM_operand_read_reg,
                                     MVM_operand_obj | MVM_operand_read_reg};
...
static void p6invokeunder(MVMThreadContext *tc, MVMuint8 *cur_op) {
  ...
  MVM_frame_special_return(tc, tc->cur_frame, return_from_fake, NULL, NULL,
                           NULL);
...
MVM_DLL_EXPORT void Rakudo_ops_init(MVMThreadContext *tc) {
  ...
  MVM_ext_register_extop(tc, "p6capturelex", p6capturelex, 2, s_p6capturelex,
                         NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6capturelexwhere", p6capturelexwhere, 2,
                         s_p6capturelexwhere, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6getouterctx", p6getouterctx, 2, s_p6getouterctx,
                         NULL, NULL, MVM_EXTOP_PURE | MVM_EXTOP_ALLOCATING);
  MVM_ext_register_extop(tc, "p6captureouters", p6captureouters, 2,
                         s_p6captureouters, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6stateinit", p6stateinit, 1, s_p6stateinit, NULL,
                         NULL, 0);
  MVM_ext_register_extop(tc, "p6setfirstflag", p6setfirstflag, 2,
                         s_p6setfirstflag, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6takefirstflag", p6takefirstflag, 1,
                         s_p6takefirstflag, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6setpre", p6setpre, 1, s_p6setpre, NULL, NULL,
                         0);
  MVM_ext_register_extop(tc, "p6clearpre", p6clearpre, 1, s_p6clearpre, NULL,
                         NULL, 0);
  MVM_ext_register_extop(tc, "p6inpre", p6inpre, 1, s_p6inpre, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6finddispatcher", p6finddispatcher, 2,
                         s_p6finddispatcher, NULL, NULL, MVM_EXTOP_NO_JIT);
  MVM_ext_register_extop(tc, "p6argsfordispatcher", p6argsfordispatcher, 2,
                         s_p6argsfordispatcher, NULL, NULL, 0);
  MVM_ext_register_extop(tc, "p6decodelocaltime", p6decodelocaltime, 2,
                         s_p6decodelocaltime, NULL, NULL, MVM_EXTOP_ALLOCATING);

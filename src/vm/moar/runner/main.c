...
static int starts_with(const char *str, const char *want) {
  ...
  return str_len < want_len ? 0 : strncmp(str, want, want_len) == 0;
}
...
static int parse_flag(const char *arg) {
  ...
  found = bsearch(arg, FLAGS, sizeof FLAGS / sizeof *FLAGS, sizeof *FLAGS,
                  cmp_flag);
...
int retrieve_home(char **out_home, const char *rel_home,
                  const size_t rel_home_size, const char *env_var,
...
#endif
{
  ...
  for (; (flag = parse_flag(argv[argi])) != NOT_A_FLAG; ++argi) {
    ...
    case OPT_DEBUGPORT: {
      ...
      if (port <= 1024 || 65535 < port) {
        fprintf(stderr,
                "ERROR: debug server port out of range. We only accept ports "
                "above 1024 and below 65535. (got: %" PRIi64 ")\n",
                port);
        return EXIT_FAILURE;
      }
  ...
  if (getenv("PERL6_HOME")) {
    if (!retrieve_home(&rakudo_home, perl6_rel_path, perl6_rel_path_size,
                       ...
                       perl6_check_path, perl6_check_path_size)) {
  ...
  if (debugserverport > 0) {
    ...
    if (start_suspended) {
      instance->main_thread->gc_status =
